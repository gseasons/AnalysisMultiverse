...
from split_half.functions import get_sink
import os
...
class split:
    ...
    def construct(self):
        ...
        inputnode = Node(IdentityInterface(fields=['corrected', 'mask', 'bold', 'covariates', 'out_frame']), name='inputnode')
        ...
        split.connect([(inputnode, split.get_node('repro'), [('corrected', 'stats'),
                       ...
                       (inputnode, split.get_node('predict'), [('bold', 'bold'),
                                                               ...
                                                               ('mask', 'mask')]),
                       (inputnode, split.get_node('distance'), [('out_frame', 'out_frame')]),
                       ])
    ...
    def distance(self, flow):
        from split_half.functions import calc
        calculation = Node(Function(input_names=['R', 'P', 'out_frame'],
                                    output_names=['score', 'score_txt'], function=calc), name='calculation')
        calculation.inputs.task = self.task
    ...
    def reproducibility(self, flow):#INPUT WILL BE LEVEL3 outputs, JOINED POST GROUP SPLITTING i.e. a bunch of pairs
        from split_half.functions import compare
    ...
    def prediction(self, flow):
        from split_half.functions import predict
