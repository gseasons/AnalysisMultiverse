                    ...
                    for key in self._flatten(result['result'].inputs):
                        if os.path.isfile(str(key)) or os.path.isdir(str(key)):
                            ...
                            if isinstance(it, dict) and key in it.values():
                                ...
                                print('MATCHED1')
                                print(jobid)
                                print(list(self.refidx[:, jobid].nonzero()[0]))
                                return
        ...
        if str2bool(self._config["execution"]["remove_node_directories"]):
            ...
            for idx in indices:
                ...
                if self.proc_done[idx] and (not self.proc_pending[idx]):
                    print("NODE DONE AND READY TO BE DELETED")
                    lst = [idx]
                    cont = True
                    searched = []
                    while cont:
                    ...
                    for i in lst:
                        if self.proc_done[i] and (not self.proc_pending[i]):#and not indices_check.any()
                            if self.procs[i] in indices_check:
                                print("CONTINUING")
                                continue
                            print("EXPECTING TO DELETE")
                            print(self.procs[i])
                            self.refidx[i, i] = -1
