...
###################  #Gra - original function ###################
def load(path, file):
    out = os.path.join(out_dir, path, file)
    # print('Mah|we are in load function| the path that is loading:', out) #Mah
...
###################  #Mah - modified function ###################

# def load(path, file):
#     out = os.path.join(out_dir, path, file)
#     print('Mah|we are in load function| the path that is loading:', out) 
    
#     if not os.path.isfile(out):
#         raise FileNotFoundError(f"File not found: {out}")
...
def save(path, file, frame):
    ...
    # print('Mah| saving a pkl file here: ', out) #Mah
    ...
    for path in pathlist:
        ...
        for i, column in enumerate(dat_frame):
            ...
            if isinstance(col, dict):
                for key in col:
                    if task == 'rest':
                        # if isinstance(key, str) and ('gamma' in key or 'dgamma' in key): #Gra Orignial, changed on oct 20,2025
                        if isinstance(key, str) and ('gamma' in key or 'dgamma' in key or 'none' in key): #Mah, changed on oct 20,2025
