...
class preprocess(spatial_normalization):
    ...
    def preprocessing(self, flow, func_dic):
        ...
        Fregress = Node(Function(input_names=input_names,
                                 output_names=['warped', 'forreho']), name='Fregress', mem_gb=3)
    ...
    def coregistration(self, flow, func_dic):#SEARCH TO SEE IF BET ALREADY RUN AND OUTPUT SAVED
        ...
        bet = Node(Function(input_names=['data_dir', 'T1w'], output_names='out_file', function=check4brains), name='bet') #init_brain_extraction_wf(name='bet')
