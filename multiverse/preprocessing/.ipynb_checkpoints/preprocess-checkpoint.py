...
class preprocess(spatial_normalization):
    ...
    def construct(self, func_dic, subjects):
        ...
        if brain_extracted:
            print('BRAIN IS INDEED EXTRRATED |MAh') #Mah
            preprocess.connect(inputnode, 'T1w', preprocess.get_node('bet'), 'T1w')
        else:
            print('BRAIN WAS NOT EXTRRATED |MAh') #Mah
    ...
    def coregistration(self, flow, func_dic, subjects):
        ...
        for sub in subjects:
            ...
            if not masked:
                from niworkflows.anat.ants import init_brain_extraction_wf
                # bet = init_brain_extraction_wf(name='bet', omp_nthreads=2, mem_gb=3) #Org #Gra
                bet = init_brain_extraction_wf(name='bet', omp_nthreads=2, mem_gb=3, atropos_use_random_seed=False) #Mah
