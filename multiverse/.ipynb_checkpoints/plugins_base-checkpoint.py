...
class DistributedPluginBase(PluginBase):
    ...
    def _load_state(self):
        # print('we are in plugin_base.py at _load_state |Mah ') #Mah
        files_missing = True
        ...
        if checkpoints:
            ...
            while files_missing:
                try:
                    with open(_recent, 'rb') as f:
                        # print('we are in plugin_base.py at checkpoints, saved_state| Mah| print checkpoints:', checkpoints) #Mah
                        saved_state = pickle.load(f)
                    files_missing = False
                except Exception as e:
                    try:
                        # print('we are in plugin_base.py at _load_state at checkpoints, while files_missing, try: | Mah|print checkpoints:', checkpoints) #Mah
            ...
            self.hours_elapsed = len(checkpoints) #Mah 
            # print(' len(checkpoints) :' , len(checkpoints)) #Mah
    ...
    def _save_state(self, stamp):
        # print('we are in plugin_base.py def _save_state| Mah') #Mah
        try:
    ...
    def run(self, graph, config, updatehash=False):
        ...
        temp = self.__dict__.copy() #mah
        task = temp['plugin_args']['task'] #mah
        batch = temp['plugin_args']['batch'] #mah
        
        file_name = '/scratch_dir/processed/reproducibility/checkpoints_' + task + '_batch_' + str(batch) #mah
        check = glob(os.path.join(file_name, 'checkpoint_*.pkl')) #Mah
        # print('we are in plugin_base | check = glob(file_name) : ', check) #Mah
        # print(' task, batch : ', task, batch )
        # if str2bool(self._config["execution"]["remove_node_directories"]):
        if check:  #if a checkpoint exist the code should go into this if statement
            # print('we are in plugins_base| run()| check ') #Mah
            self._load_state()
        self.STOP = False
            ...
            while self.pending_tasks:
                ...
                try:
                    ...
                    if not self.STOP:
                        ...
                        print('we are here except Exception as exc: if not self.STOP: _save_state |Mah')  #Mah
                    ...
                    if result:
                        if result["traceback"]:
                            if not self.STOP:
                                ...
                                self._save_state(self.hours_elapsed)
                                print('we are here if result["traceback"]: if not self.STOP: _save_state |Mah')  #Mah
                            ...
                            ##### Mah ### added this on Nov5 ################## to save a checkpoint every ~5 minutes ####################
                            if (time() - self.timestamp) / 3600 > 0.5:  ###### Mah changed it to this: (Nov5,2025)  # 0.5 = every 30 mintunes
                                self.hours_elapsed += 1
                                self._save_state(self.hours_elapsed)
                                self.timestamp = time()
    ...
    def _remove_node_dirs(self):
        ...
        if str2bool(self._config["execution"]["remove_node_directories"]) and not self.STOP:
            ...
            if (time() - self.timestamp) / 3600 > 0.5: ####Original Gra code
            # if (time() - self.timestamp) / 3600 > 0.08:  ###### Mah changed it to this: (Nov4,2025) # 0.08 ~ every 5 mintunes
                self.hours_elapsed += 1
                self._save_state(self.hours_elapsed)
                self.timestamp = time()
