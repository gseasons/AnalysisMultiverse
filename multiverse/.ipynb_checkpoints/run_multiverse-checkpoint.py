...
print('we are in run_multiverse.py') #Mah
exp_dir = '/scratch_dir'
...
if sys.argv[1] == "True":
    config['rerun'] = True
    print('run_multiverse.py: rurun was TRUE |Mah') #Mah #This was correct! we are in rerun
else:
    config['rerun'] = False
    print('run_multiverse.py: rurun was FALSE |Mah') #Mah
...
def check_pipes(): 
    ...
    for task in tasks:
        try:
            frame = load('', task+'.pkl')
            unique.append(frame.astype(str).drop_duplicates(subset=frame.columns.difference(['R', 'P', 'Score'])).shape[0] > config['pipelines']) #Gra
...
def on_pop_gen(ga):
    ...
    # checkpoints = glob.glob('/scratch_dir/processed/reproducibility/checkpoints/checkpoint_*.pkl') #Gra
    
    ##### changed to this, Nov 5, 2025 ########### #Mah
    # checkpoints = True #changed to true to force the code to go through the ifs #Mah

    # print('Mah| checkpoints: ', checkpoints) #Mah 

    # if config['rerun'] or checkpoints: #Gra
    if config['rerun']: #mah nov5,2025
        # print("should be here in  if config['rerun'] or checkpoints: ") #Mah #we are here during rerun
        ...
        if type(is_params) != str:
            ...
            pipeline_ = generation * pop_.shape[0]
            solution_start = pipeline_
            # print('pipeline_ : ', type(pipeline_), pipeline_) #Mah #we do go here during rerun
    else:
    ...
    for task in tasks:
        ...
        # if config['rerun'] or checkpoints: #gra
        if config['rerun']: #mah 
            frame = ''  #Mah #we are here during rerun #i thinnk the problem is here! why is this should be empty? 
        else:
            frame = load('', task+'.pkl')
        ...
        for batch in range(iterations):
            checkpoints = glob.glob('/scratch_dir/processed/reproducibility/checkpoints_' + task + '_batch_' + str(batch)+ '/checkpoint_*.pkl') #Mah -added nov5,2025
            if checkpoints:
                print('if this is not a rerun for a specific job that already containts checkpoints, you shouldnt be seeing this!!!') #mah
                workflows = glob.glob('/scratch_dir/processed/reproducibility/' + task + '_workflow*')
                last_batch = len(workflows) - 1
                # print('in run_multiverse| last_batch: ', last_batch, 'batch: ', batch) #Mah
                if batch < last_batch:
                    continue
            ...
            out_frame = save('', task+'.pkl', unique_pipelines) #Mah #this is running during re-run
            ...
            if 'anat' in types and 'func' in types and to_run:
                if config['processing'] == 'SLURM' and 'num_generations' not in config:
                    ...
                    print('we are in anat in run_multiverse.py |mah dec2 2025| save_dir: ', save_dir) #Mah
                    ...
                    if os.path.exists(save_dir + '_done') and config['rerun']:
                        ...
                        print('we are in if anat.. if config[rerun] |mah nov28 2025') #mah
                        # shutil.rmtree(wf_path) #mah / i just commented this out- dec2,2025/ i dont want to delete these folders
                    elif config['rerun'] and os.path.exists(save_dir) and glob.glob(save_dir + '/crash-*'):
                        for crash in glob.glob(save_dir + '/crash-*'):
                            os.remove(crash)
                    else:
                        # save('reproducibility', task + '_workflow_' + str(batch) + '.pkl', pipelines) #Gra 
                        print('we are in if anat... else |mah') #mah
                        save('reproducibility', task + '_workflow_' + str(batch) + '.pkl', pipelines) #Mah. #this is running during re-run mode
    ...
    if 'num_generations' not in config:
        print("we are in run_multiverse|  if 'num_generations' not in config:| IM COMMENTING OUT THE NEXT IF") #Mah
        # if checkpoints:
