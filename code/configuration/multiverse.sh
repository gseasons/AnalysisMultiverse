...
srun singularity run -B $2:/scratch -e -B ~/code:$custom_base -B ~/.ipython:/scratch/.ipython -B ~/code:/code/multiverse -B $1:/data $container ipengine --profile=${profile} --location=$(hostname) --log-to-file &
sleep 45
echo "Launching Job"
...
singularity exec -H $2:/scratch -e -B ~/code:$custom_base -B ~/.ipython:/scratch/.ipython -B ~/code:/code/multiverse -B $1:/data \
