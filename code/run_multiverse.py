...
def on_pop_gen(ga):
    ...
    for task in tasks:
        ...
        if 'anat' in types and 'func' in types and to_run:
            ...
            pipelines.run(plugin=config, plugin_args=plugin_args)#plugin='Linear', plugin_args={'status_callback': log_nodes_cb})#config['processing'])
    ...
    if 'num_generations' not in config:
        ...
        #TODO: PASS IN NUMBER OF TASKS AND RUNTIME AND GB TO MULTIVERSE.SH (~pipelines x ~subjects) -> needs to be environment variables I think
        #      TEST THAT DELETE FILES WHEN DONE WORKS WITH IPYTHON
